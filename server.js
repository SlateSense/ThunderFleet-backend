...
const startCronJob = () => {
  ...
  const task = cron.schedule('* * * * *', () => { // Every minute
    ...
    Object.keys(games).forEach(gameId => {
      ...
      if (game.state === 'waiting' && game.players.length === 1) {
      ...
      } else if (game.state === 'waiting' && game.players.length === 2) {
        game.state = 'placing';
        ...
        console.log(`[Server] Game ${gameId} started with players ${game.players.join(', ')}`);
      }
    });
  }, {
    scheduled: true,
...
setInterval(checkCronStatus, 60000); // Check every minute
